...
class sources:
    ...
    def getSources(self, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, timeout=30):
        ...
        if content == 'movie':
            sourceDict = [i for i in sourceDict if i.endswith(('_movies', '_movies_series'))]
        else:
            sourceDict = [i for i in sourceDict if i.endswith(('_series', '_movies_series'))]

        try: sourceDict = [(i, control.setting('provider.' + re.sub('_movies_series$|_movies$|_series$', '', i))) for i in sourceDict]
        ...
        if content == 'movie':
            ...
            for source in sourceDict: threads.append(workers.Thread(self.getMovieSource, title, year, imdb, re.sub('_movies_series$|_movies$|_series$', '', source), __import__(source, globals(), locals(), [], -1).source()))
        else:
            tvshowtitle = cleantitle.normalize(tvshowtitle)
            for source in sourceDict: threads.append(workers.Thread(self.getEpisodeSource, title, year, imdb, tvdb, season, episode, tvshowtitle, premiered, re.sub('_movies_series$|_movies$|_series$', '', source), __import__(source, globals(), locals(), [], -1).source()))
        ...
        sourceLabel = [re.sub('_movies_series$|_movies$|_series$', '', i) for i in sourceDict]
    ...
    def sourcesFilter(self):
        ...
        local_lang = [i for i in self.sources if 'lang' in i and not i['lang'] == '']
        self.sources = [i for i in self.sources if not i in local_lang]
        ...
        filter = []
        filter += local
        if quality == '0': filter += [i for i in local_lang if i['quality'] == '1080p']
        if quality == '0' or quality == '1': filter += [i for i in local_lang if i['quality'] == 'HD']
        ...
        filter += [i for i in local_lang if i['quality'] == 'SD']
        filter += [i for i in local_lang if i['quality'] == 'SCR']
        filter += [i for i in local_lang if i['quality'] == 'CAM']
        ...
        for i in range(len(self.sources)):
            ...
            try: l = self.sources[i]['lang']
            except: l = self.sources[i]['lang'] = ''
            ...
            if not l == '': label += ' | [COLOR FF008000]%s[/COLOR]' % (l)
