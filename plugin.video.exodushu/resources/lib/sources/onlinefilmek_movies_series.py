...
class source:
    ...
    def movie(self, imdb, title, year):
        try:
            ...
            originaltitle = re.sub('(?:\().+', '', t).strip()
            try: originaltitle = originaltitle.encode('utf-8')
            except: pass
            title = originaltitle.replace(' ', '+')
            
            query = urlparse.urljoin(self.base_link, self.search_link % title)
            r = client.request(query)
            if not r: raise exception()
            result = json.loads(r)

            result = [i for i in result if cleantitle.get(i['nev'].encode('utf-8')) == cleantitle.get(originaltitle)]
    ...
    def episode(self, url, imdb, tvdb, title, premiered, season, episode):
        try:
            ...
            if not r: raise exception()
            result = json.loads(r)
            result = [i for i in result if cleantitle.get(re.sub('(?:\s\d\.\s*\xe9vad)\s*', '', i['nev']).encode('utf-8')) == cleantitle.get(originaltitle)]
        ...
        try:
            ...
            for i in result:
                try:
                    ...
                    if i[0] == 'kep-magyar_szinkron': lang = 'szinkron'
                    else: lang = ''
