...
class source:
    ...
    def movie(self, imdb, title, year):
        try:
            ...
            query = urllib.urlencode({'f[g][text]': t, 'f[g][tags][]': year_id})
            ...
            result = [i[1] for i in result if cleantitle.get(t) == cleantitle.get(i[0].encode('utf-8')) or cleantitle.get(title) == cleantitle.get(i[0].encode('utf-8'))]
        ...
        try:
            ...
            for item in result:
                try:
                    ...
                    sources.append({'source': host, 'quality': quality, 'lang': 'szinkron', 'provider': 'Filmecske', 'url': url, 'direct': False, 'debridonly': False})
                except:
                    pass
            return sources
        except:
